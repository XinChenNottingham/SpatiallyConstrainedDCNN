...
class SimpleTFModel(Model):
    def __init__(self, net, x_suffix, y_suffix, m_suffix=None, dropout=0, loss_function={'cross-entropy': 0.5, 'dice':0.5}):
        super().__init__(net)
        ...
        self._loss_dict = {'cross-entropy': LF.cross_entropy,
                           ...
                           'sc-loss': LF.spatially_constrained_loss}
    ...
    def predict(self, data_dict):
        logits = self.net(data_dict[self._x_suffix])
        prob = tf.nn.softmax(logits, -1)
        return prob
